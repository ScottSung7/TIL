...
### 1. Mocking 과 실제 동작이 다른 경우나 나타났다. DB를 이용할 때는 UUID를 자동 추가 하는데 내가 해준 모킹은 그렇지 않았다. 매번 모킹 동작을 만들어야 하다 보니 내가 만든 모킹이 실제 동작과 다를 경우 False Positive 에 취약해 진다는 생각이 들었다.
...
그래서 Stub 클래스를 만들어 사용하는 것이 **반복되는 같은 모킹 제작에서 실수를 할 가능성을 더 줄여 준다는 생각**이다.
